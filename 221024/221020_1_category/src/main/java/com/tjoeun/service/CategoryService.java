...
public class CategoryService {
	...
	// CategoryDAO 클래스와 서브 카테고리를 저장하는 insert sql 명령을 실행하는 메소드를 호출하는 메소드
	public void reply(CategoryVO vo) {
		System.out.println("CategoryService의 reply() 메소드");
		SqlSession mapper = MySession.getSession();
		CategoryDAO dao = CategoryDAO.getInstance();
		
		// 서브 카테고리가 삽입될 위치를 결정하기 위해서 lev와 seq를 각각 1씩 증가시긴다.
		// 서브 카테고리의 레벨은 부모 카테고리 레벨보다 크다.
		vo.setLev(vo.getLev() + 1);
		
		// 서브 카테고리가 부모 카테고리 바로 아래 나와야 하므로 출력 순서는 부모 카테고리보다 1커야 한다.
		vo.setSeq(vo.getSeq() + 1);
		
		// 서브 카테고리를 위치에 맞게 삽입하기 위해서 캍은 카테고리 그룹(gup)의 카테고리 출력 순서(seq)를 조정/수정(update)하는 메소드 실행
		HashMap<String, Integer> hmap = new HashMap<>();
		hmap.put("gup", vo.getGup());
		hmap.put("seq", vo.getSeq());
		dao.increment(mapper, hmap); // updeate
		
		// 서브 카테고리를 테이블에 저장(insert)하는 메소드를 호출
	...
	// delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 
	// CategoryDAO 클래스의 카테고리 1건을 얻어오는 delete sql 명령을 실행하는 메소드를 호출하는 메소드
	public void delete(int idx) {
		System.out.println("CategortService의 delete() 메소드");
		SqlSession mapper = MySession.getSession();
	...
	public void deleteCategory(int idx) {
		System.out.println("CategortService의 deleteCategory() 메소드");
		SqlSession mapper = MySession.getSession();
	...
	public void deleteCheck(int idx) {
		System.out.println("CategortService의 deleteCheck() 메소드");
		SqlSession mapper = MySession.getSession();
	...
	// restoreCheck 필드의 값을 "NO"로 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드
	public void restoreCheck(int idx) {
		System.out.println("CategortService의 restoreCheck() 메소드");
		SqlSession mapper = MySession.getSession();
		
		CategoryDAO.getInstance().restoreCheck(mapper, idx);
	...
	public void update(CategoryVO vo) {
		System.out.println("CategortService의 update() 메소드");
		SqlSession mapper = MySession.getSession();
	...
	// report.jsp에서 호출되는 신고할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 클래스의 카테고리 1건의
	// deleteReport 필드의 값을 1증가(수정) 시키는 update sql 명령을 실행하는 메소드를 호출하는 메소드
	public void report(int idx) {
		System.out.println("CategortService의 report() 메소드");
		SqlSession mapper = MySession.getSession();
	...
	// CategoryDAO 클래스의 삭제할 카테고리 목록을 얻어오는 delete sql 명령을 실행하는 메소드를 호출하는 메소드
	public ArrayList<CategoryVO> deleteList(CategoryVO vo) {
		System.out.println("CategortService의 selectList() 메소드");
		SqlSession mapper = MySession.getSession();
			
		ArrayList<CategoryVO> deleList = CategoryDAO.getInstance().deleteList(mapper, vo);
	...
	// CategoryDAO 클래스의 삭제할 카테고리 그룹에 seq를 0부터 1씩 증가하게 다시 부여하는 
	// delete sql 명령을 실행하는 메소드를 호출하는 메소드
	public void reSeq(int gup) {
		System.out.println("CategortService의 reSeq() 메소드");
		SqlSession mapper = MySession.getSession();
		CategoryDAO dao = CategoryDAO.getInstance();
		
		// seq를 다시 부여할 카테고리 그룹만 얻어와서 ArrayList에 저장
		...
		// ArrayList로 얻어온 카테고리 그룹의 카테고리 개수만큼 반복하며 seq를 0부터 다시 부여
		for (int i = 0; i < gupList.size(); i++) {
			// seq를 수정할 카테고리 일련번호(idx)와 seq를 수정할 데이터(i)를 HashMap 객체에 저장
			HashMap<String, Integer> hmap = new HashMap<>();
