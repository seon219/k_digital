...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// ConfigurableEnvironment 인터페이스 타입의 객체에서 스프링 컨테이너의 환경 설정 정보를 
		// getEnvironment() 메소드로 얻어와서 ConfigurableEnvironment 인터페이스 객체에 저장한다.
		ConfigurableEnvironment env = ctx.getEnvironment();

		// properties 정보를 저장하는 MutablePropertySources 클래스 객체에 ConfigurableEnvironment 인터페이스 타입의 객체에서 
		// getPropertySources() 메소드로 properties 정보만 얻어와서 저장한다.
		MutablePropertySources mutablePropertySources = env.getPropertySources();
		
		try {
			// 스프링 컨테이너에서 얻어온 properties 정보가 저장되는 MutablePropertySources 클래스 객체 mutablePropertySources에서
			// addLast() 메소드로 맨 마지막 위체에 admin.properties 파일의 properties 내용을 읽어 추가한다.
			// MutablePropertySources 클래스 객체 mutablePropertySources에는 properties가 저장될 때 key에 따라 value 값이 저장되는 Map 형태로 저장된다.
			mutablePropertySources.addLast(new ResourcePropertySource("classpath:admin.properties"));
			// 스프링 컨테이너 환경에 추가된 properties 파일의 내용은 ConfigurableEnvironment 인터페이스 객체에서 getProperty() 메소드로 읽어올 수 있다.
		...
		System.out.println("admin.id: " + adminConnection.getAdminId()); // asdfg
		System.out.println("admin.pw: " + adminConnection.getAdminPw()); // 12345
