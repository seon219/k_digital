...
//		상태이다. => 생성자가 실행되지 않은 상태이다. => bean이 생성되지 않은 상태
...
//		bean이 생성되고 refresh() 메소드를 실행하지 않으면 getBean() 메소드로 컨테이너에서 bean을
//		얻어내는 순간 bean이 생성된다.
...
//		메소드는 실행되지 않눈다.
//		close() 메소드를 실행했더라고 refresh() 메소드를 실행하지 않았으면 destroy() 메소드는 실행되지 않는다.
		ctx.close();
