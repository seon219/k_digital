...
<body>
	...
	<%
		...
		MultipartRequest multipartRequest = new MultipartRequest(
				request, 							
				"D:/upload/", // 보안성 향상을 위해 upload 폴더를 webapp 폴더 외부에 만든다.
				5 * 1024 * 1024 * 1024, 			
				"UTF-8", 							
		...
		// hasMoreElements(): Enumeration 인터페이스 객체에 저장된 다음 데이터가 있으면 true, 없으면 false
		while(filenames.hasMoreElements()) { // 업로드한 파일이 있는동안 반복
			...
			// 업로드 할 파일이 넘어오지 않았으면 다음 파일을 처리한다. => 남은 반복문을 실행할 필요 없다.
			if (filename == null) {
				continue;
			}
			...
			// 업로드 제한
			File realfilename = new File("D:/upload/" + fileRealname);
			...
			if (fileLength > 5 * 1024 * 1024) {
				...
				out.println("alert('업로드 파일 용량 초과')");
				realfilename.delete();
				...
				*/
				...
				// filename과 fileReanmaned을 테이블에 저장하는 메소드를 호출
				//new FileDAO();
