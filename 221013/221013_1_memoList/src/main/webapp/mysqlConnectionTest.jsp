...
<body>
	...
	<% 
		...
		try {
			...
			// mysql에 연결
			// DriverManager클래스의 getConnection(url, user, password) 메소드로 mysql에 연결한다.
			// url에 사용할 mysql이 설치된 경로와 데이터베이스 이름을 적는다.
			String url = "jdbc:mysql://localhost:3306/javaam?useUnicode=true&charactorEncoding=UTF-8"; // 5.x 사용자
			// String url = "jdbc:mysql://localhost:3306/javaam?serverTimeZone=UTC"; // 8.x 사용자
			// localhost는 mysql이 설치된 컴퓨터가 현재 사용중인 컴퓨터임을 읨
			// localhost 대신 127.0.0.1을 사용해도 된다.
			// 3306은 mysql이 컴퓨터와 통신할 때 사용하는 포트 번호 의미 => 오라클은 1521 사용
