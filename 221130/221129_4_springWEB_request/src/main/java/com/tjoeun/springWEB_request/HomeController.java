...
public class HomeController {
	...
	public String memberInsert( HttpServletRequest request, Model model) {
		...
		// xml 파일을 이용해서 MenberVO 클래스의 bean을 생성하고 setter 메소드로 데이터를 넣어준다. 
	...
	*/
	...
	//  클래스 객체에 setter 메소드를 사용해서 저장한 다음 Model 인터페이스 객체에 넣어주는 동작이 일괄적으로 처리된다.
	// 커멘드 객체는 뷰 페이지에서 컨트롤러로 넘어온 데이터를 저장할 클래스를 의미하는 뷰 페이지로 넘어가는 객체의 이름은
	//  커맨드 객체 이름의 첫 문자만 소문자로 변경되서 넘어간다.
	...
	*/
	...
	// @ModelAttribute 어노테이션을 이용해서 별도로 지정해서 사용할 수 있다.
	// 이 경우 기존 커맨드 객체 이름은 뷰 페이지에서 사용할 수 었고 @ModelAttribute 어노테이션으로 지정한 이름을 사용해야 한다.
	public String memberInsert( HttpServletRequest request, Model model, 
			/*MemberVO memberVO*/
