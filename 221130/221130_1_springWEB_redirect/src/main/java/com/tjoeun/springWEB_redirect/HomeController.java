...
public class HomeController {
	...
	public String confirm(HttpServletRequest request, Model model) {
		...
		// id가 "abc"이면 idOK.jsp를 브라우저에 표시하고 "abc"가 아니면 idNG.jsp를 브라우저에 표시한다.
		if(id.trim().equals("abc")) {
			
			// 아래와 같이 그냥 리턴시키면 "/WEB-INF/views" + "idOK" + ".jsp"가 연결되서 브라우제어 idOK.jsp가 나타난다.
			// return "idOK"; // 뷰 페이지 이름
			
			// 아래와 같이 "redirect:"를 붙여서 리턴시키면 "/WEB-INF/views" 폴더의 confirmOK.jsp를 호출하는 것이 아니고
			// 컨트롤러의 @RequestMapping("/confirmOK") 어노테이션이 붙어있는 메소드를 호출시킨다.
