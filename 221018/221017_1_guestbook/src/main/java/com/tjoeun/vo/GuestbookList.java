...
public class GuestbookList {
	...
	// 1페이지 분량의 글(GuestbookVO) 목록을 기억할 List를 선언
	ArrayList<GuestbookVO> list = new ArrayList<>();
	
	// 페이징 작업에 사용할 8개의 변수를 선언
	...
	// pageSize, totalCount, currentPage를 인수를 넘겨받아 초기화시키고,
	// 나머지 5개의 변수를 초기화 할 값을 계산해서 초기화시키는 생성자
	public GuestbookList(int pageSize, int totalCount, int currentPage) {
		...
		// pageSize, totalCount, currentPage를 제외한 나머지 변수를 계산해서 초기화시키는 메소드를 호출
		calculator();
	}
	...
	// ====================== getter & setter =====================================
	public ArrayList<GuestbookVO> getList() {return list;}
	public void setList(ArrayList<GuestbookVO> list) {this.list = list;}
