...
public class SelectService implements MvcboardService {
	...
	public void execute(Model model) {
		...
		int pageSize = 10;
		// HttpServletRequest 인터페이스 객체에 저장되서 넘어온 화면에 표시할 페이지 번호를 받는다.
		int currentPage = 1;
		...
		logger.info("테이블에 저장된 전체 글의 개수는 {}개 입니다.", totalCount);
		...
		// MvcboardList 클래스의 bean을 얻어온 후 8개의 변수를 초기화시키는 메소드를 실행한다.
