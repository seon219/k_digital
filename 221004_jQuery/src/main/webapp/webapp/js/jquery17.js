$(() => {
	...
	$('button').eq(1).click(() => {
		// append(): 인수로 지정된 요소를 선택된 요소 내부에 마지막 요소로 추가
	...
	$('button').eq(2).click(() => {
		$('div').html('<marquee> html 요소를 변경 </marquee>');
	})
	
	
	$('button').eq(3).click(() => {
		$('div').text('<marquee> text 요소를 변경 </marquee>')	;
