...
class Food {
	...
	// 클래서 멤버 함수 선언 시 앞에 function을 붙이지 않는다.
	constructor(name) {
...
chicken.addBrands('bhc치킨');
chicken.addBrands('푸라닭치킨');
...
class Animal { // 부모 클래스
	...
	// 클래스의 멤버 함수를 만들면 자바스크립트가 자동으로 prototype에 등록
	say() {
...
// 상속은 java와 같이 extends 예약어를 사용
class Dog extends Animal {
	constructor(name, sound, age) {
		// 부모클래스에서 상속받은 멤버 변수를 초기화하기 위해서 부모 클래스의 생성자를 호출
