function addAppend() {
	...
	//p.innerText = '<b>fieldset 내부의 div 태그 뒤에 추가한다</b>'
...
function addBefore() {
	...
	//p.appendChild(txt);
	// testContent: createTextNode() 함수로 문자열을 만들고 appendChild() 함수를 실행한 것처럼 태그 내부에 문자열을 만든다.
	p.textContent = '<b>fieldset 내부의 div 태그 앞에 추가한다</b>'
	
	// insertBefore(a, b): b태그 앞에 a 태그를 삽입
...
function moveElement() {
	...
	// children[index]: 선택된 태그의 index번째 해당되는 자식 요소를 얻어온다.
	console.log(fieldset.children[0]);
	console.log(fieldset.children[1]);
	
	// 특정 태그를 선택해서 다른 위치에 넣어주면 기존 위치에서 제거	
