function evalTest() {
	...
	if (confirm('연산식을 올바르게 입력했나요?\n 입력한 연산식: ' + str.value)) {
		// eval(): 인수로 지정된 문자열 형태의 자바스크립트 코드를 실행
		//document.getElementsByTagName('span')[0].innerHTML = '<h2> 계산결과: ' + eval(str.value) + '</h2>';
		document.querySelectorAll('span')[0].innerHTML = '<h2> 계산결과: ' + eval(str.value) + '</h2>';		
	} else {
		str.value = '';
		str.focus();
	}
...
function calTest() {
	...
	document.querySelectorAll('span')[1].innerHTML = '<h2> 계산결과: ' + eval(str) + '</h2>';	
}
