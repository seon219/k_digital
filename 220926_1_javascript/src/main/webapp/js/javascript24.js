function numberObject() {
	...
	// document.write(): 인수로 지정한 내용을 브라우저의 내용을 전부 지우고 출력
	// document.write('메롱');
	...
	// parseInt(): 문자열이나 실수를 정수로 변환
	divs[1].innerHTML = '100' + 5; //연결
	divs[2].innerHTML = parseInt('100') + 5; // 덧셈
	
	// '100a'와 같이 숫자로 시작하고 정수로 변환할 수 없는 문자열이 뒤에 나오면 변환할 수 있는 위치까지 정수로 변환
	divs[3].innerHTML = parseInt('100a') + 5; 
	// 'a100'과 같이 정수로 변환할 수 없는 문자열이 맨 앞에 나오면 정수로 변환할 수 있는 숫자가 아니므로 NaN을 리턴
