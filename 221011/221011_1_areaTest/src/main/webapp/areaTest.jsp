...
<body>
	...
	<% 
		...
		// 영역 변수가 저장되는 기억공간은 영역 변수 이름을 key로 하고 영역 변수에 저장되는 데이터를 value로 하는
		// Map<String, Object> 형태의 구조로 value의 자료형이 Object인 이유는 개발자가 어떤 타입의 데이터를 영역 변수에 저장할 지 모르기 때문
		...
		// 영역 변수에 저장된 데이터는 저장할 데이터 타입으로 반드시 형변환시켜서 사용해야 한다.
		...
		// sendRedirect() 메소드 주소창에 표시되는 페이지 이름이 인수로 지정된 페이지 이름으로 변경된다.
		// 현재 페이지와 sendRedirect() 메소드의 인수로 지정한 페이지가 완전히 분리됨
		// 		=> 새로고침하면 주소창에 표시된 페이지만 다시 실행
		response.sendRedirect("requestTest.jsp");
		
		
		// forward(): 인수로 지정된 페이지로 넘겨준다.
		// forward() 메소드는 request 영역의 데이터를 가지고 인수로 지정된 페이지로 넘어간다.
		// forward() 메소드 주소팡에 표시되는 페이지 이름이 forward() 메소드가 실행된 페이지 이름이 그대로 유지
		// 현재 페이지와 forward() 메소드의 인수로 지정한 페이지가 연결된 상태 
		//		=> 새로고침 하면 주소창에 표시된 forward() 메소드가 입력된 페이지부터 현재 화면에 보이는 페이지까지 실행
