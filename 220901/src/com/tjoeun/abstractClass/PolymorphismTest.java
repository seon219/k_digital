package com.tjoeun.abstractClass; // 추상메소드 - 다형성

import java.util.Scanner;

// 다형성(Polymorphism): 같은 메소드가 서로 다른 클래스에서 다양하게 실행되는 것
// 다형성을 구현하기 위해서는 다형성을 구현할 메소드가 포함된 모든 클래스가 같은 부모를 가져야 하고,
// 부모와 자식 클래스에 같은 이름의 메소드가 있어야 하며 자식 클래스는 부모클래스의 메소드를 반드시 Override 시켜서 사용해야 함
// 부모 클래스 타입의 객체에 자식 클래스 타입의 객체를 대입해서 다형성이 구현된 메소드를 실행
