...
public class HomeController {
	...
	// value 속성(생략가능)에 지정된 내용과 주소창에 요청된 내용이 같은 @RequestMapping 어노테이션이 붙어있는 메소드가 자동으로 실행된다.
	
	// method 속성으로 RequestMethod.GET이 지정되면 get 방식의 요청만 받아서 처리할 수 있고, 
	// RequestMethod.POST가 지정되면 post 방식의 요청만 받아서 처리할 수 있다. => 요청방식이 일치하지 않으면 405 에러가 발생된다.
	// @RequestMapping(value = "/", method = RequestMethod.GET)

	// @RequestMapping 어노테이션에서 method 속성을 생략하면 get 방식과 post 방식을 구분하지 않고,
	// 모든 요청을 받아 처리할 수 있다.
	// method 속성을 생략하면 value 속성만 남게되고 이 경위 value 속성도 생략하고 요청만 적으면 된다.
	@RequestMapping("/")
	// @RequestMapping 어노테이션이 붙어있는 메소드는 필요한 작업을 실행하고 view 페이지(jsp 파일)의 이름을 리턴시킨다. 
	...
	public String home(Locale locale, Model model) {
		...
		// view 페이지 이름 앞에 위치하는 경로명과 뒤에 위치하는 확장명은 servlet-contect.xml 파일에 설정되어 있다.
