...
<body>
	...
	<%
		...
		// enctype이 multipart/form-data로 설정된 폼에서 넘어오는 데이터를 받으려면 
		// 일반적인 request로 받을 수 없고 multipart를 지원하는 객체로 받아야 한다.
		try{
			MultipartRequest mr = new MultipartRequest(
										request, 							// 요청 객체
										application.getRealPath("/upload"), // 파일이 업로도 될 실제 경로
										5 * 1024 * 1024, 					// 업로드 할 파일의 최대 크기를 바이트 단위로 지정
										"UTF-8", 							// 문자 인코딩 방식
										new DefaultFileRenamePolicy() 		// 업로드 되는 파일과 같은 이름의 파일이 존재할 경우 이름을 자동으로 변경해주는 객체
								  );
			
		// 업로드하는 원본 파일의 이름을 얻어온다.
		...
		out.println("디스크에 실제 업로드된 파일 이름: " + realFilename.getName() + "<br/>" );
		
		} catch (IOException e) {
			out.println("파일 용량 초과, 5MB까지만 업로드 가능");
		}
	%>
