...
<body>
	...
	<%
		...
		// increment.jsp에서 넘어오는 조회수를 증가시킨 메인글의 글번호, 돌아갈 페이지 번호, 분기할 페이지 이름을 받는다.
		int idx = Integer.parseInt(request.getParameter("idx"));
		int currentPage = Integer.parseInt(request.getParameter("currentPage"));
		String job = request.getParameter("job");
		
		FreeboardService service = FreeboardService.getinstance();
		// 메인글 1건을 얻어오는 메소드를 호출
		...
		// job에 contectView가 넘어왔을 경우 댓글 목록을 읽어와서 request 영역에 저장시킨다.
		...
		// contentView.jsp에서 넘어온 경우 메인글을 수정(update.jsp), 삭제(delete.jsp)하는 페이지로 
		// request 영역에 저장된 내용을 가지고 넘겨준다.
