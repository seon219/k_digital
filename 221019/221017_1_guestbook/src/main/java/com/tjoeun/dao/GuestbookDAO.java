...
public class GuestbookDAO {
	...
	// selectService 클래스에서 호출되는 mapper와 화면에 표시할 페이지의 시작 인덱스 번호와 끝 인덱스 번호가 저장된 HashMap 객체를 넘겨받고,
	...
	// selectService 클래스에서 호출되는 mapper와 수정 또는 삭제할 글번호를 넘겨받고
	// guestbook.xml 파일의 수정 또는 삭제할 글 1건을 얻어오는 select sql 명령을 실행하는 메소드
	...
	// deleteService 클래스에서 호출되는 mapper와 삭제할 글번호를 넘겨받고
	// guestbook.xml 파일의 글 1건을 삭제하는 delete sql 명령을 실행하는 메소드
	...
	// updateService 클래스에서 호출되는 mapper와 수정할 정보가 저장된 객체를 넘겨받아
	//  guestbook.xml 파일의 글 1건을 수정하는 update sql 명령을 실행하는 메소드
	...
	// selectService 클래스에서 호출되는 mapper와 검색어(내용)를 넘겨받고 
	// guestbook.xml 파일의 테이블에 저장된 전체 글 중에서 검색어를 포함하는 글의 개수를 얻어오는 select sql 명령을 실행하는 메소드
	...
	//  selectService 클래스에서 호출되는 mapper와 화면에 출력할 페이지의 시작 인덱스와 끝 인덱스, 검색어(내용)가 저장된
	// param 클래스 객체를 넘겨받고 guestbook.xml 파일의 내영에 검색어를 포함하는 1페이지 분량의 글 목록을 얻어오는 select sql 명령을 실행하는 메소드
